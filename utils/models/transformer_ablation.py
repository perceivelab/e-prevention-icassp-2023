...
class Embeddings(nn.Module):
    ...
    def __init__(self, input_features, output_features, output_size, dropout=0.25):
        ...
        self.input_embeddings = Embedding(input_features, output_features, output_size)
    ...
    def forward(self, x):
        ...
        #py compute position embeddings
...
class AblationModel(nn.Module):
    ...
    def __init__(self, input_features:int, d_model: int, seq_len: int = 512, dropout: float = 0.5):
        super().__init__()
...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.classification_head = torch.nn.Sequential(
            Linear(self.d_model*self.seq_len, 512),
            nn.Sigmoid(),
    ...
    def forward(self, src: Tensor) -> Tensor:
        if len(src.size()) == 4:
            src = src.squeeze(0)
        flatten = self.ablation(src).view(src.size(0), -1)
        #print(cls_token.size())
        logits = self.classification_head(flatten)
        #print(logits.size())
